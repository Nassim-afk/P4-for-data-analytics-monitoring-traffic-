...
class MonitorController():  
    ...
    def __init__(self, sw_name):
        ...
        self.topo = Topology(db="topology.db") #le controleur va chercher les informations liée à la topologie du réseau
        self.sw_name = sw_name #le nom des switchs
        self.thrift_port = self.topo.get_thrift_port(sw_name) #les ports de connexion
        self.controller = SimpleSwitchAPI(self.thrift_port)

    def recv_msg(self, msg):
        f=open("storage.txt", "a") #ouvre un fichier texte comme base de données"""
        ...
        for sub_message in range(num):
            ...
            random_num,ethertype,ttl,protocol, src,dst = struct.unpack(("!BHBBII"), msg[0:offset]) #extrait les champs d'en-tête qu'on souhaite traiter
            SRC_port=struct.unpack(("!H"), msg[13:15])
            DST_port=struct.unpack(("!H"), msg[15:17])
            MAC_src=struct.unpack(("!HI"), msg[17:23])
            MAC_dst=struct.unpack(("!HI"), msg[23:29])
            ...
            y= str(datetime.datetime.now()),"random number:", random_num,"ethertype",hex(ethertype),"ttl",ttl,"protocole",protocol,"src ip:", str(ipaddress.IPv4Address(src)),"dst ip:", str(ipaddress.IPv4Address(dst)),"port source:",SRC_port,"port destination:",DST_port
            msg = msg[offset:]
            y=str(y) #transformer les données en text 
            f.write ("********************Monitoring on sw1 **********************" #écriture dans le fichier
            +"\n"+y+"\n")
    ...
    def run_loop(self): #création de socket de connexion pour lire le contenu des paquets transitant dans le réseau 
